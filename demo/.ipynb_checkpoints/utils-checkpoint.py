...
def HistRoutine(feed_dict,xlabel='',ylabel='',reference_name='true',logy=False,binning=None,label_loc='best'):
    ...
    for ip,plot in enumerate(feed_dict.keys()):
        ...
        if reference_name!=plot:
            ...
            dist_error = np.std(dist, ddof = 1) / np.sqrt(np.size(dist))
            reference_hist_error = np.std(reference_hist, ddof = 1) / np.sqrt(np.size(reference_hist))
            error = 100 * np.absolute(np.divide((dist * reference_hist_error - reference_hist * dist_error), reference_hist * reference_hist, where = reference_hist != 0))
            if 'steps' in plot or 'r=' in plot:
                #ax1.plot(xaxis,ratio,color=colors[plot],marker=line_style[plot],ms=10,lw=0,markeredgewidth=3)
                ax1.errorbar(xaxis,ratio,yerr=error,ecolor=colors[plot],elinewidth=1,capsize=1,color=colors[plot],marker=line_style[plot],ms=10,lw=0,markeredgewidth=3)
            else:
                #ax1.plot(xaxis,ratio,color=colors[plot],marker='o',ms=10,lw=0,markerfacecolor='none',markeredgewidth=3)
                ax1.errorbar(xaxis,ratio,yerr=error,ecolor=colors[plot],elinewidth=1,capsize=1,color=colors[plot],marker='o',ms=10,lw=0,markerfacecolor='none',markeredgewidth=3)
