...
class GSGM_Model(keras.Model):
    """Score based generative model"""
    def __init__(self, num_particles = 2, name='SGM',sde_type='VPSDE',config=None):
        super(GSGM_Model, self).__init__()
    ...
    def prior_sde(self,dimensions, boost = lambda x: x):
        #print(dimensions)
        if self.sde_type == 'VESDE':
            return boost(tf.random.normal(dimensions)*self.sigma_1)
        else:
            return boost(tf.random.normal(dimensions))
    ...
    def PCSampler(self,
        ...
        init_x = self.prior_sde(data_shape, boost = boost)
